...
class ROPNet(nn.Module):
    ...
    def forward(self, src, tgt, num_iter=1, train=False):
        ...
        for b in range(batch_num):
            ...
            split_indices_list = []
            weight_list = []
            indice_num = src_t_batch.shape[1]
            ...
            indices = torch.arange(indice_num)
            split_indices = indices[torch.randperm(indice_num)[:int(0.8 * indice_num)]]
        ...
        for b, (indices, weight) in enumerate(zip(batch_indices_list, batch_weight_list)):
            ...
            for i, w in zip(indices, weight):
                ...
                R_split, t_split = R_cur_split @ R[b:b + 1, :, :], R_cur_split @ t[b:b + 1, :, None] + t_cur_split[:, :,
                                                                                                       None]
